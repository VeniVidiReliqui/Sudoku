...
public:
  ...
  /* REQUIRED FOR LAB */
  /* Prints the current board state in an easy-so understand way. If solve hasn't been
  ...
  /* Prints the current board state.  If solve hasn't been called, returns the starter
   * board, otherwise a solved board. */
  void printState();
...
private:
  ...
  bool checkCompletelyFilled(const board_t& state);
  bool isValid(const board_t& state);
  bool checkRow(const board_t& state);
  bool checkCol(const board_t& state);
  bool checkBox(const board_t& state);
